...
public class ScreenFirst extends AppCompatActivity {
    ...
    protected Dialog onCreateDialog(int id){
        if(id == DIALOG_ID1)
            return new TimePickerDialog(ScreenFirst.this,TimePickerListener1, hour_x, minute_x,true);
        if(id == DIALOG_ID2)
            return new TimePickerDialog(ScreenFirst.this,TimePickerListener2, hour_y, minute_y,true);
        return null;
    }
    ...
    protected TimePickerDialog.OnTimeSetListener TimePickerListener2 = new TimePickerDialog.OnTimeSetListener() {
        ...
        public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
            hour_y = hourOfDay;
            minute_y = minute;
            Toast.makeText(ScreenFirst.this,hour_y+":"+minute_y,Toast.LENGTH_LONG).show();
    ...
    public void compareTime(int hour_x,int hour_y,int c_hour,int minute_x,int minute_y,int c_min){
        ...
        if((c_hour >= hour_x) && (c_hour <= hour_y)) {
            Toast.makeText(ScreenFirst.this, "Silent Mode on", Toast.LENGTH_LONG).show();
            am.setRingerMode(0);
        }
        else {
            Toast.makeText(ScreenFirst.this,"Silent Mode off",Toast.LENGTH_LONG).show();
            am.setRingerMode(2);
        }
    }
}
